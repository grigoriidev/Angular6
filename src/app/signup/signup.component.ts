    ...
    signupUser() {
        ...
        var datas = this.httpService.httpPost(url, userData).subscribe(
            data => {
                console.log(data);
                if (data['status'] == 'true') {
                    Swal('Federal candiate information has been updated successfully.');
                    this.router.navigate(['/federal-candidate-home']);
                } else {
                    Swal('Unable to update federal candiate information.');
                }
...
changeType(){
    ...
    numberOnlyDPI(event): boolean {
        ...
        if (charCode > 31 && (charCode < 48 || charCode > 57)) {
            ...
            this.validation = 'The value should lie in 0 to 100';
